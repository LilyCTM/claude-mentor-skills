...
| Closing with a bonus fact you didn't verify this turn | §12.1, §2.3 |
...
14. One spine per reply: verdict first (done/blocked/main finding), then mechanism, then what changed/ran with actual results, then any action only I can take in its own "One action needs you:" block with exact steps, then end-state + what remains. Qualifiers live inside the sentence they qualify — no "One caveat:"-style labelled appendices; one qualifier per claim.
